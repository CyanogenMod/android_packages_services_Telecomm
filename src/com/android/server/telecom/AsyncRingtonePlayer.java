...
class AsyncRingtonePlayer {
            ...
            public void handleMessage(Message msg) {
                switch(msg.what) {
                    ...
                    case EVENT_INCREASE_VOLUME:
                        mCurrentIncrementVolume += 0.05F;
                        Log.d(AsyncRingtonePlayer.this,
                                "Increasing ringtone volume to " + mCurrentIncrementVolume);
                        mRingtone.setVolume(mCurrentIncrementVolume);
                        if (mCurrentIncrementVolume < 1F) {
                            sendEmptyMessageDelayed(EVENT_INCREASE_VOLUME, mIncrementInterval);
                        }
                        break;
                }
            }
    ...
    private void handlePlay(Uri ringtoneUri, float incStartVolume, int incRampUpTime) {
        ...
        if (incRampUpTime > 0) {
            ...
            int steps = (int) Math.ceil((1F - incStartVolume) / 0.05F);
            mIncrementInterval = incRampUpTime * 1000 / steps;
            mCurrentIncrementVolume = incStartVolume;

            mHandler.sendEmptyMessageDelayed(EVENT_INCREASE_VOLUME, mIncrementInterval);
        } else {
