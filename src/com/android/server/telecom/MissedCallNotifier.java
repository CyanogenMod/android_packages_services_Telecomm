...
class MissedCallNotifier extends CallsManagerListenerBase {
    ...
    void fetchCallInfoAsync(final CallInfo call) {
        if (mMissedCallCount.get() > 1 || call.getCallerInfo().contactExists
        ...
        mCallInfoExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                if (hasInfo && mMissedCallCount.get() == 1) {
    ...
    void showMissedCallNotification(Call call) {
        ...
        showMissedCallNotificationInternal(info);
    }
     ...
     */
    synchronized void showMissedCallNotificationInternal(CallInfo call) {
        if (call != mLastCallInfo) {
